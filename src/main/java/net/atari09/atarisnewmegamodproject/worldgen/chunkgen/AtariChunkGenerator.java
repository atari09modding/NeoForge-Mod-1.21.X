...
public class AtariChunkGenerator extends ChunkGenerator {

    public static final MapCodec<AtariChunkGenerator> CODEC = RecordCodecBuilder.mapCodec((p_255585_) ->
            p_255585_.group(BiomeSource.CODEC.fieldOf("biome_source")
                    ...
                    .forGetter((p_224278_) -> p_224278_.settings)).apply(p_255585_, p_255585_.stable(AtariChunkGenerator::new)));
    ...
    public AtariChunkGenerator(BiomeSource biomeSource, Holder<NoiseGeneratorSettings> settings) {
        super(biomeSource);
    ...
    private int sampleHeight(int x, int z, RandomState randomState) {
        ...
        NormalNoise spikyNoise = randomState.getOrCreateNoise(ModNoises.SPIKY);
        ...
        double noiseSpikyValue = spikyNoise.getValue((double) x *3,0, (double) z *3);

        int continentalness = (int)(Math.round(noiseContinentalnessValue * 40));
        int jagged = (int)(Math.round(noiseJaggedValue * 8));
        int spiky = (int)(Math.round((noiseSpikyValue-0.7)*500));
        if(spiky<0){
            spiky = (int)(Math.round((noiseSpikyValue-0.5)*100));
        }
        spiky = spiky > 0 ? spiky : 0;



        return BASE_HEIGHT + continentalness + jagged + spiky ;
    }
    ...
    public void addDebugScreenInfo(List<String> list, RandomState randomState, BlockPos blockPos) {
        ...
        NormalNoise spiky = randomState.getOrCreateNoise(ModNoises.SPIKY);
        ...
        list.add("spiky:" + spiky.getValue(blockPos.getX(),0,blockPos.getZ()));
    }
