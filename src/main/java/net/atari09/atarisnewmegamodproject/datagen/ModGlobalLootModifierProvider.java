...
import net.atari09.atarisnewmegamodproject.loot.AddItemModifier;
...
public class ModGlobalLootModifierProvider extends GlobalLootModifierProvider {
    public ModGlobalLootModifierProvider(PackOutput output, CompletableFuture<HolderLookup.Provider> registries) {
        super(output, registries, AtariMod.MOD_ID);
    }

    @Override
    protected void start() {
        this.add("radis_seeds_to_short_grass",
                new AddItemModifier(new LootItemCondition[]{
                        LootItemBlockStatePropertyCondition.hasBlockStateProperties(Blocks.SHORT_GRASS).build(),
                        LootItemRandomChanceCondition.randomChance(0.25f).build() }, ModItems.RADISH_SEEDS.get()));
        this.add("radis_seeds_to_tall_grass",
                new AddItemModifier(new LootItemCondition[]{
        ...
        this.add("magma_core_from_trial_vault",
                new AddItemModifier(new LootItemCondition[]{
                        new LootTableIdCondition.Builder(ResourceLocation.withDefaultNamespace("chests/reward_ominous_unique")).build(),
                }, ModItems.MAGMA_CORE.get()));
